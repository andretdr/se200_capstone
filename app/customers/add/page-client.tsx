...
const schema = z.object({
  ...
  first_name: z.string().min(3, "First name must be at least 2 characters").max(30, "First name cannot exceed 20 characters"),
  last_name: z.string().min(3, "Last name must be at least 2 characters").max(30, "Last name cannot exceed 20 characters"),
  
  policies: z.string().min(1, "Please select a policy"),
});
...
export default function AddCustomerPage(policies: Policies) {
  ...
  const handleRegistration = async (formData) => {
    ...
    const typeString = formData.policies;
    let policyID = '';

    for (let item of policies.data){
        if (item.policy_name === typeString)
